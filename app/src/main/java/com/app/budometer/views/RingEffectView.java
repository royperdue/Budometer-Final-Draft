...
public class RingEffectView extends View {
    ...
    public void setAngle(@FloatRange(from = 0.0, to = 360.0) float angle) {
        ...
        final int stepCount = (int) (diff / BudometerConfig.STEP_DEGREE);
        final float stepMod = diff % BudometerConfig.STEP_DEGREE;

        final float sw = mPaint.getStrokeWidth() * 0.5f;
        final float radius = mRadius - sw;

        for (int i = 1; i <= stepCount; i++ ) {
            final float stepAngel = mStartAngle + mAngle + BudometerConfig.STEP_DEGREE * i;
            final float x = (float) Math.cos(Math.toRadians(stepAngel)) * radius;
            final float y = (float) Math.sin(Math.toRadians(stepAngel)) * radius;
            mPath.lineTo(x, y);
        }

        final float stepAngel = mStartAngle + mAngle + BudometerConfig.STEP_DEGREE * stepCount + stepMod;
        final float x = (float) Math.cos(Math.toRadians(stepAngel)) * radius;
        final float y = (float) Math.sin(Math.toRadians(stepAngel)) * radius;
        mPath.lineTo(x, y);
