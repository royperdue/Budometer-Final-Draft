 ...
 */
public class BaseInnerLayoutManager extends RecyclerView.LayoutManager
    ...
    public void onAttachedToWindow(RecyclerView view) {
        if (!(view instanceof BaseInnerRecyclerView)) {
    ...
    private void applyTransformation() {
        for (int i = 0, cnt = getChildCount(); i < cnt; i++) {
            ...
            if (view.getBottom() > 0) {
                ...
                ((BaseInnerItem) mRecyclerView.getChildViewHolder(view)).onItemViewHeightChanged(viewNewHeight);
            }

            if (view.getTop() > 0) {
                ((BaseInnerItem) mRecyclerView.getChildViewHolder(view)).resetInnerLayoutOffset();
            }
        }
    }
}
