...
public class CircleMenuView extends FrameLayout {
    ...
    public CircleMenuView(@NonNull Context context, @NonNull List<Integer> icons, @NonNull List<Integer> colors) {
        ...
        initLayout(context);
        initMenu(R.color.item_separator_color);
        initButtons(context, icons, colors);
    }
    ...
    private void initLayout(@NonNull Context context) {
        LayoutInflater.from(context).inflate(R.layout.view_circle_menu, this, true);
        ...
        final float density = context.getResources().getDisplayMetrics().density;
        final float buttonSize = BudometerConfig.DEFAULT_BUTTON_SIZE * density;

        mRingRadius = (int) (buttonSize + (mDistance - buttonSize / 2));
        mDesiredSize = (int) (mRingRadius * 2 * BudometerConfig.DEFAULT_RING_SCALE_RATIO);
    ...
    private void initButtons(@NonNull Context context, @NonNull List<Integer> icons, @NonNull List<Integer> colors) {
        ...
        for (int i = 0; i < buttonsCount; i++) {
            ...
            if (i == 0)
                button.setTag("Gallery");
            else if (i == 1)
                button.setTag("Crop");
            else if (i == 2)
                button.setTag("Chart");
            else if (i == 3)
                button.setTag("Carousel");
            else if (i == 4)
                button.setTag("Help");
            else if (i == 5)
    ...
    private Animator getButtonClickAnimation(final @NonNull FloatingActionButton button) {
        ...
        final ObjectAnimator scaleX = ObjectAnimator.ofFloat(mRingView, "scaleX", 1f, BudometerConfig.DEFAULT_RING_SCALE_RATIO);
        final ObjectAnimator scaleY = ObjectAnimator.ofFloat(mRingView, "scaleY", 1f, BudometerConfig.DEFAULT_RING_SCALE_RATIO);
    ...
    private Animator getOpenMenuAnimation() {
        final ObjectAnimator alphaAnimation = ObjectAnimator.ofFloat(mMenuButton, "alpha", BudometerConfig.DEFAULT_CLOSE_ICON_ALPHA);
    ...
    public void configureButtons(int index, int icon) {
        for (int i = 0; i < mButtons.size(); i++) {
            if (mButtons.get(i).getTag().equals("Gallery") && index == 0) {
                ((FloatingActionButton)mButtons.get(i)).setImageResource(icon);
            } else if (mButtons.get(i).getTag().equals("Crop") && index == 1) {
                ((FloatingActionButton)mButtons.get(i)).setImageResource(icon);
            }  else if (mButtons.get(i).getTag().equals("Chart") && index == 2) {
                ((FloatingActionButton)mButtons.get(i)).setImageResource(icon);
            }  else if (mButtons.get(i).getTag().equals("Carousel") && index == 3) {
                ((FloatingActionButton)mButtons.get(i)).setImageResource(icon);
            }  else if (mButtons.get(i).getTag().equals("Help") && index == 4) {
                ((FloatingActionButton)mButtons.get(i)).setImageResource(icon);
            } else if (mButtons.get(i).getTag().equals("Camera") && index == 5) {
                ((FloatingActionButton)mButtons.get(i)).setImageResource(icon);
            }
        }
    }
    ...
    private void openOrClose(boolean open, boolean animate) {
        ...
        if (animate) {
            ...
            mMenuButton.setAlpha(open ? BudometerConfig.DEFAULT_CLOSE_ICON_ALPHA : 1f);
